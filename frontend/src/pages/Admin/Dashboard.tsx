...
/** Only accept JSON arrays for feed — API errors become [] so the tab never crashes. */
async function fetchFeedEntries(): Promise<FeedEntry[]> {
  const res = await fetch(`${API}/local/feed`)
  if (!res.ok) return []
  try {
...
export default function AdminDashboard() {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(async () => {
      setFeed(await fetchFeedEntries())
    }, 10_000)
              ...
              return (
                ...
                >
                  ...
                  <p className="text-xs text-white/25 mt-2">
                    Generated by CompletionAgent · {formatFeedTimestamp(entry.created_at)}
                  </p>
                </motion.div>
              )
            })}
